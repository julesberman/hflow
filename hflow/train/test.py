...
def get_hist(frame, nx=72):
    frame = frame.T
    H, x, y = jnp.histogram2d(frame[0], frame[1], bins=nx, range=[[0,1],[0,1]])
    return H
...
def plot_test(true_sol, test_sol, t_int, n_plot, mu_i):
    ...
    scatter_movie(plot_sol, t=t_int, c=cs, alpha=0.3, xlim=[0,1], ylim=[0,1], show=False, frames=frames, save_to=f'{outdir}/sol_{mu_i}.gif')
    
    # plot hist
    idx = np.linspace(0, len(test_sol)-1, frames, dtype=np.int32)
    hist_sol = get_hist(test_sol[idx])
    imshow_movie(hist_sol, t=t_int[idx], show=False,  frames=frames, save_to=f'{outdir}/test_hist_{mu_i}.gif')
