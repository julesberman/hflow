...
#########
# CODE MODIFIED FROM: https://github.com/n2cholas/jax-resnet/tree/main
#########
...
class Reshaper(nn.Module):
    ...
    @nn.compact
    def __call__(self, x, mu_t):
        t = mu_t[-1:]
        xy = x.shape[-1]
        nn = int(xy**0.5)
        x = rearrange(x, '(X Y) -> X Y', X=nn)
        t = jnp.ones_like(x)*t
        x_t = jnp.concatenate([x[..., None], t[..., None]], axis=-1)
        x_t = x_t[None]
        return x_t
...
def ResNet(
    ...
    *,
    ...
    reshaper = Reshaper()
    layers = [reshaper, stem_cls(), pool_fn]
...
ResNeSt50FastSmall = partial(ResNet, stage_sizes=STAGE_SIZES[18],
